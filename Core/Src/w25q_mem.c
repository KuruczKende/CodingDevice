...
W25Q_STATE W25Q_EraseSector(u32_t SectAddr) {
    /*if (SectAddr >= SECTOR_COUNT * MEM_SECTOR_SIZE * 1024)
        return W25Q_PARAM_ERR;
*/
    while (W25Q_IsBusy() == W25Q_BUSY)
        w25q_delay(1);
...
W25Q_STATE W25Q_WaitForWritingComplete(){
    W25Q_STATE state;
    uint8_t sr;
    uint32_t k=0;
    do {
        state = W25Q_ReadStatusReg(&sr,1);
        k+=1;
    }   while (state!=W25Q_OK && k < 100);
    return state;
}

W25Q_STATE W25Q_EnableMemoryMappedMode(void){
    QSPI_CommandTypeDef com = {0};
